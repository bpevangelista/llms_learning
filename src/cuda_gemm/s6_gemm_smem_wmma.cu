...
__global__ void gemm_kernel_m16n8k16(const half* __restrict__ mat_a_f16, const half* __restrict__ mat_b_f16, half* __restrict__ mat_out_f16) {
    ...
    constexpr int32_t kMatrixBatch = 2;
    ...
    constexpr int32_t kSmemLengthK_U32 = kMatrixBatch * 8;  // 128B
    ...
    // TODO Add per-k padding to avoid bank conflicts
    #pragma unroll
    for (int32_t i=0; i < 4; i++) { // 64B copy per-thread
    ...
    for (int32_t count=0, k=0; k < kMatSizeK; count++, k += kKernelSizeMK) {
        ...
        if (count % kMatrixBatch == 0) {
            ...
            for (int32_t i = 0; i < 4; i++) {
                int32_t local_row = tid_k + i * 4;
                cp_async16B_cache_global(&smem_mat_a_u32[kernel_index][write_index][local_row * kSmemLengthK_U32 + tid_ele * kCopyLength_U32],
                    &mat_a_u32[(row + local_row) * kGmemLengthK_U32 + tid_ele * kCopyLength_U32 + (k + kKernelSizeMK * kMatrixBatch) / 2]);
            }
        ...
        const uint32_t* __restrict__ a_u32 = &smem_mat_a_u32[kernel_index][read_index][mat_shift];
        const uint32_t* __restrict__ b_u32 = &smem_mat_b_u32[kernel_index][read_index][mat_shift];
...
int main() {
    ...
    return gemm_main<half, float, kMatSizeM, kMatSizeN, kMatSizeK>(); // 6.2ms on 3060TI, MSE 0.0007
    //return gemm_main<float, float, kMatSizeM, kMatSizeN, kMatSizeK>(); // Not implemented yet
}
