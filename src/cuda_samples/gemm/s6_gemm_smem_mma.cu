...
template <
    ...
    int32_t matSizeM, int32_t matSizeN, int32_t matSizeK,              // matrices size
    int32_t kernelSizeM, int32_t kernelSizeN, int32_t kernelSizeK      // kernel computation size
    >
__global__ void gemm_kernel(const T* __restrict__ matA, const T* __restrict__ matB, T* __restrict__ matOut) {

    //
    int32_t row = (blockIdx.x * blockDim.x + threadIdx.x) * kernelSizeM;
    int32_t col = (blockIdx.y * blockDim.y + threadIdx.y) * kernelSizeN;
    // Out Matrix (matSizeM x matSizeN) is not evenly divisible by kernel size
    ...
    // L1 shared mem for matrix blocks
    __shared__ T sharedMatA[kernelSizeM*kernelSizeK];
    __shared__ T sharedMatB[kernelSizeN*kernelSizeK];
    // Register file for accumulators
    Acc acc[kernelSizeM*kernelSizeN] = {0}; // 16x8 acc for 16x16 * 8x16^T. Run twice, 16x8:16x8 == 16x16
    ...
    for (int32_t k=0; k < matSizeK; k += kernelSizeK) {
        ...
        //for (int32_t block=0; block < 16; block+=8) { // or += 8 when assembly

            // compute 16x16 * 16x8
            for (int i=0; i<kernelSizeM; i++) {
                for (int j=0; j < kernelSizeN; j++) {
                    for (int innerK=0; innerK < kernelSizeK; innerK++) {
                        acc[i * kernelSizeN + j] += static_cast<Acc>(
                            matA[(row+i) * matSizeK + k + innerK] *
                            matB[(col+j) * matSizeK + k + innerK]);
                    }
                }
            }
...
//             acc += static_cast<Acc>(
//                 matA[row * matSizeK + k + block] *
//                 matB[col * matSizeK + k + block]);
        //}
...
__global__ void gemm_kernel_mn(const T* __restrict__ matA, const T* __restrict__ matB, T* __restrict__ matOut) {
    ...
    // L1 thread group shared mem for tiles
    __shared__ T sharedMatA[kernelSizeM*kernelSizeK];
    __shared__ T sharedMatB[kernelSizeN*kernelSizeK];
