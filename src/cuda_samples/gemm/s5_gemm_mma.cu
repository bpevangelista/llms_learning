...
__global__ void gemm_kernel_m16n8k16(const half* __restrict__ mat_a, const half* __restrict__ mat_b, half* __restrict__ mat_out) {
    ...
    for (int32_t k=0; k < kMatSizeK; k += kKernelSizeMK) {
        ...
        uint32_t* mat_a_top_u32 = (uint32_t*)&mat_a[(row + shift_k + 0) * kMatSizeK + k];
        uint32_t* mat_a_bot_u32 = (uint32_t*)&mat_a[(row + shift_k + 8) * kMatSizeK + k];
        uint32_t a0 = mat_a_top_u32[shift_elem];
        uint32_t a1 = mat_a_bot_u32[shift_elem];
        uint32_t a2 = mat_a_top_u32[shift_elem + 4]; // step 8 - 4x(2xf16)
        uint32_t a3 = mat_a_bot_u32[shift_elem + 4]; // step 8 - 4x(2xf16)

        // MatB 16x8 tile
        // 4xf16 across 2 32b register (2xf16 p/register)
        uint32_t* mat_b_u32 = (uint32_t*)&mat_b[(col + shift_k + 0) * kMatSizeK + k];
        uint32_t b0 = mat_b_u32[shift_elem];
        uint32_t b1 = mat_b_u32[shift_elem + 4]; // step 8 - 4x(2xf16)
